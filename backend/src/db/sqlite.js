...
// Insert seed data (only if products table is empty)
const count = db.prepare('SELECT COUNT(*) as count FROM products').get();
if (count.count === 0) {
  const insert = db.prepare(`
    INSERT INTO products (slug, title, description, price, file_key, file_name, download_limit, category, tags, is_bundle, image_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const products = [
...
// Wrap SQLite to mimic pg pool interface
export const pool = {
  query: (text, params) => {
    try {
