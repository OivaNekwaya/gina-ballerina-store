...
router.get('/:token', downloadLimiter, async (req, res) => {
  ...
  try {
    const tokenRes = await pool.query(`
      ...
      WHERE dt.token = ? AND dt.expires_at > datetime('now') AND dt.remaining_downloads > 0
    `, [token]);
    ...
    await pool.query(`UPDATE download_tokens SET remaining_downloads = remaining_downloads - 1 WHERE token = ?`, [token]);
